...
from game import actions, colour, exceptions
...
# Refer to http://rogueliketutorials.com/tutorials/tcod/v2/part-10/ for context
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self,
        ...
        """Default: any key exits this input handler."""
        if event.sym in {
                tcod.event.K_LSHIFT,
                tcod.event.K_RSHIFT,
                tcod.event.K_LCTRL,
    ...
    def on_exit(self) -> Optional[ActionOrHandler]:
        ...
        Default: this returns to the main event handler.
        """
        return MainGameEventHandler(self.engine)
...
class CharacterScreenEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        console.print(
            x=x + 1, y=y + 2,
            string=f"XP: {self.engine.player.level.current_xp}/{self.engine.player.level.experience_to_next_level}"
            )
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    Behaviour then depends on the subclass.
    """
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        key = event.sym
        if key in MOVE_KEYS:
            modifier = 1 # Holding mod keys speed up movement
...
class MainGameEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        if key in MOVE_KEYS:
            ...
            return LogViewer(self.engine, GameState.ACTIVE)
            # return LogViewer(self.engine)
...
class GameOverEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> None:
        ...
        if event.sym == tcod.event.K_ESCAPE:
            ...
            return LogViewer(self.engine, GameState.DEAD)


class LogViewer(EventHandler):
    """Print the log's history on a larger window
    which can be navigated
    """

    def __init__(self, engine: Engine, game_state: GameState):
        super().__init__(engine)
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[MainGameEventHandler]:
        ...
        if event.sym in CURSOR_Y_KEYS:
            ...
            if adjust < 0 and self.cursor == 0:
                ...
                self.cursor = max(
                    0,
                    min(self.cursor + adjust, self.log_length -1)
                    )
        elif event.sym == tcod.event.K_HOME:
            self.cursor = 0 # Move directly to the top
        elif event.sym == tcod.event.K_END:
            self.cursor = self.log_length - 1 # All the way to the bottom
        else: # other keys return to game state
            # return MainGameEventHandler(self.engine)
            # to main game state if still alive
            if self.game_state is GameState.ACTIVE:
                return MainGameEventHandler(self.engine)
            # to game over state if dead
            elif self.game_state is GameState.DEAD:
                return GameOverEventHandler(self.engine)
        return None
